...
less temp.fa | awk 'BEGIN {OFS="\t"} {if(substr($1,1,1)==">"){print $0} else{gsub(/[a-z]/,"N",$0);print $0}}' > temp_no_repeats.fa
...
less ./fimo_out/fimo.gff | grep "^chr"| awk 'BEGIN{OFS="\t"} {split($1,chr,":"); split(chr[2],loc,"-"); offset = int(($4+$5)/2); print chr[1],loc[1]+offset,loc[1]+offset+1,$9}' > centered_motif_locations.bed
